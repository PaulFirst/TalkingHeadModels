...
def generate_landmarks(frames_list, face_aligner):
    ...
    for i in range(len(frames_list)):
        try:
            ...
            fig.canvas.draw()
            fig.set_figheight(224 / dpi)
            fig.set_figwidth(224 / dpi)
            # plt.show()
            #plt.savefig('fig')
            # plt.draw()
    ...
    for j in range(len(frames_list) - len(frame_landmark_list)):
        # filling frame_landmark_list in case of error
        frame_landmark_list.append(frame_landmark_list[j])
...
def select_images_frames(path_to_images):
    ...
    for image_name in os.listdir(path_to_images):
        #print((path_to_images + "/" + image_name).replace('/', '\\').replace('.jpg', '.jpeg'))
...
def generate_landmarks(image_path, device, pad):
    ...
    while (no_pic == True):
        ...
        for i in range(len(frames_list)):
            try:
                ...
                ax.plot(preds[0:17, 0], preds[0:17, 1], marker='', markersize=5, linestyle='-', color='green', lw=2)
                # left and right eyebrow
                ax.plot(preds[17:22, 0], preds[17:22, 1], marker='', markersize=5, linestyle='-', color='orange',
                        lw=2)
                ax.plot(preds[22:27, 0], preds[22:27, 1], marker='', markersize=5, linestyle='-', color='orange',
                        lw=2)
                # nose
                ax.plot(preds[27:31, 0], preds[27:31, 1], marker='', markersize=5, linestyle='-', color='blue',
                        lw=2)
                ax.plot(preds[31:36, 0], preds[31:36, 1], marker='', markersize=5, linestyle='-', color='blue',
                        lw=2)
                # left and right eye
                ax.plot(preds[36:42, 0], preds[36:42, 1], marker='', markersize=5, linestyle='-', color='red', lw=2)
                ax.plot(preds[42:48, 0], preds[42:48, 1], marker='', markersize=5, linestyle='-', color='red', lw=2)
                # outer and inner lip
                ax.plot(preds[48:60, 0], preds[48:60, 1], marker='', markersize=5, linestyle='-', color='purple',
                        lw=2)
                ax.plot(preds[60:68, 0], preds[60:68, 1], marker='', markersize=5, linestyle='-', color='pink',
                        lw=2)
                ax.axis('off')
