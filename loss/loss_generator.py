...
class LossCnt(nn.Module):
    def __init__(self, VGGFace_body_path, VGGFace_weight_path, device):
        ...
        self.vgg19_caffe_RGB_mean = torch.tensor([123.68, 116.779, 103.939], dtype=torch.float).view(1, 3, 1, 1).to(
            device)  # RGB order
        self.vggface_caffe_RGB_mean = torch.tensor([129.1863, 104.7624, 93.5940], dtype=torch.float).view(1, 3, 1, 1).to(
            device)  # RGB order
